    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const todayOrders = workOrders.filter(
          (wo) => new Date(wo.createdDate).toDateString() === today,
        );

        setStats({
          todayWorkOrders: todayOrders.length,
          pendingTasks: workOrders.filter(
            (wo) => wo.status === 'Pending' || wo.status === 'Assigned',
          ).length,
          completedToday: todayOrders.filter((wo) => wo.status === 'Completed')
            .length,
                    ...
                    <tr key={wo.workOrderId}>
                      ...
                      <td>
                        <button
                          className='btn btn-sm btn-outline-primary'
                          onClick={() =>
                            (window.location.href = `/technician/maintenance/${wo.workOrderId}`)
                          }
                        >
